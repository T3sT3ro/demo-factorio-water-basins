...
interface BasinManagerInterface {
  getBasinTiles(id: string): string[];
}

export class BasinLabelManager {
  basinLabels: Map<string, BasinLabel>;
  lastBasinHash: string | null;
  ...
  generateBasinLabels(basins: Map<string, BasinData>, _heights: number[][], pumps: PumpData[] = [], basinManager: BasinManagerInterface | null = null): void {
    ...
    basins.forEach((_basin, id) => {
      ...
      if (tiles.length === 1) {
        ...
        representativeTile = tiles[0]!; // fallback
      }
  ...
  draw(ctx: CanvasRenderingContext2D, basins: Map<string, BasinData>, heights: number[][], pumps: PumpData[] = [], zoom: number = 1, basinManager: BasinManagerInterface | null = null): void {
    ...
    this.basinLabels.forEach((label) => {
      ...
      const centerHeight = heights[centerY] && heights[centerY][centerX] !== undefined ? heights[centerY][centerX] : 0;
  ...
  createBasinHash(basins: Map<string, BasinData>, pumps: PumpData[] = []): string {
